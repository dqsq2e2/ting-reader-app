...
public class CapacitorMusicControls extends Plugin {
	...
	public void create(PluginCall call) {
			...
			// initialize(); // Removed unconditional init
			...
			try{
				...
				if (options.has("duration")) {
					long duration = options.getLong("duration") * 1000; // Convert to ms
				...
				if (options.has("elapsed")) {
					try {
						// Try double first as it might be float in JS
						elapsed = (long)(options.getDouble("elapsed") * 1000);
					} catch (Exception e) {
						elapsed = options.getLong("elapsed") * 1000;
					}
				}
	...
	public void initialize() {
		...
		setMediaPlaybackState(PlaybackStateCompat.STATE_PAUSED, PlaybackStateCompat.PLAYBACK_POSITION_UNKNOWN, 0);
	...
	public void updateIsPlaying(PluginCall call) {
		...
		try{
			...
			// We don't have elapsed time here, so we assume UNKNOWN or last known?
			// Ideally we should pass elapsed here too, but to be safe let's assume unknown for now 
			// or better, keep the last position? 
			// Since updateElapsed handles both, we should encourage using updateElapsed instead.
			// But for backward compat, let's use UNKNOWN but with correct state.
			// BETTER: Just use 0 if not provided, or PlaybackStateCompat.PLAYBACK_POSITION_UNKNOWN
			
			long elapsed = PlaybackStateCompat.PLAYBACK_POSITION_UNKNOWN;
			if (params.has("elapsed")) {
				try {
					elapsed = (long)(params.getDouble("elapsed") * 1000);
				} catch (Exception e) {
					elapsed = params.getLong("elapsed") * 1000;
				}
			}
	...
	private void setMediaPlaybackState(int state, long position, float speed) {
		...
		long actions = PlaybackStateCompat.ACTION_PLAY_PAUSE | 
						...
						PlaybackStateCompat.ACTION_SEEK_TO; // Add SEEK_TO
