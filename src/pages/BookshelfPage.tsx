...
const BookshelfPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const libs = Array.isArray(libsRes.data) ? libsRes.data : (libsRes.data.libraries || []);
      setLibraries(libs);
      ...
      setSeries(Array.isArray(seriesRes.data) ? seriesRes.data : (seriesRes.data.series || []));
    } catch (err) {
  ...
  const scrollToGroup = (key: string) => {
    setActiveLetter(key);
    // Use getElementById directly, but handle the case where key might have special chars
    // HTML id attributes can contain special characters in modern browsers, but querySelector fails without escape.
    // document.getElementById is actually safer for exact ID matches including '#'
    const element = document.getElementById(`group-${key}`);
    
    if (element) {
      // Use scrollIntoView which is more reliable across browsers than window.scrollTo with offset calculations
      element.scrollIntoView({ behavior: 'smooth', block: 'start' });
      // If we need a header offset, scrollIntoView might be blocked by fixed headers. 
      // Let's adjust it by scrolling a bit up immediately after.
      setTimeout(() => {
        ...
        <>
          ...
          {groupedItems && (
            <>
              ...
              <div 
                className="fixed right-1 top-1/2 -translate-y-1/2 z-[100] flex flex-col items-center bg-transparent py-2 select-none"
                onTouchStart={(e) => {
                  e.preventDefault();
          ...
          {visibleGroupedItems ? (
                 ...
                 <div key={key} id={`group-${key}`}>
                   ...
                   <div className={`grid ${getGridCols()}`}>
                     {visibleGroupedItems.groups[key].map(item => (
                       'books' in item ? (
                         !isSelectionMode ? <SeriesCard key={item.id} series={item as Series} coverShape={coverShape} /> : null
                       ) : (
            ...
            <div className={`grid ${getGridCols()}`}>
              {!isSelectionMode && visibleSeries.map((s) => (
