...
const StatCard = ({ icon, label, value, unit = '' }: { icon: React.ReactNode, label: string, value: string | number, unit?: string }) => {
  ...
  const handleExpand = (e: React.MouseEvent) => {
    if (isTruncated || isExpanded) {
        // If expanded, clicking the card toggles it off (or we can rely on backdrop)
        // User requirement: "click any position to collapse" -> implies backdrop handles collapse.
        // "Don't just click card to collapse".
        // So clicking card while expanded should probably do nothing or toggle?
        // Let's make it toggle for intuitive behavior, but backdrop handles outside.
        e.stopPropagation();
  ...
  return (
    <>
      ...
      <div 
        className={`bg-white dark:bg-slate-900 p-4 md:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex items-center gap-3 md:gap-4 relative group transition-all ${isExpanded ? 'z-50 scale-105' : ''} ${isTruncated ? 'cursor-pointer' : ''}`}
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="relative">
              <p 
                ref={textRef}
                className={`text-lg md:text-xl font-bold dark:text-white ${isExpanded ? 'absolute top-0 left-0 bg-white dark:bg-slate-900 p-2 shadow-xl rounded-lg border border-slate-100 dark:border-slate-800 whitespace-nowrap z-50 min-w-full' : 'truncate'}`}
              >
              {value}
